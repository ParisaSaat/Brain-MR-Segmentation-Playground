...
def make_dataset(dir, fold, dataset_letter):
    ...
    img_dir = os.path.join(dir + dataset_letter + '/slices/' + fold + '/images/')
    msk_dir = os.path.join(dir + dataset_letter + '/slices/' + fold + '/masks/')
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root, sample, fold='train', dataset_letter='A', loader=default_loader, trim_bool=0, return_path=False, random_transform=False, channels=1):
        ...
        imgs = make_dataset(root, fold, dataset_letter)
        imgs.sort(key=lambda x: x['file'], reverse=False)
    ...
    def __getitem__(self, index):
        ...
        resize_to = (256, 256) # do resize or center crop?
        ...
        msk[msk <= (msk.max() / 2)] = 0 # binarize or not?
        msk[msk >  (msk.max() / 2)] = 1
        ...
        if self.channels == 1: # normalize or not??
